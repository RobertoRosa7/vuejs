<template>
    <li class="list-group-item d-flex">
        <span>{{ tarefa.titulo }}
            <span v-if="tarefa.concluido" :class="marqueConcluido"><small>(concluído)</small></span>
            <span v-else :class="marqueConcluido"><small>(pendente)</small></span>
        </span>
...
export default {
    ...
    computed: {
        ...
        marqueConcluido(){
            return this.tarefa.concluido ? 'text-success' : 'text-warning';
        }
    },
    methods:{
        concluirTarefa(){
            // emitir um evento para component pai, com atualização da propriedade 'concluir' passando uma
